...
import { fetchLessons } from '../../apis/lessons.client';
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setLessons([...data, ...custom]);
      } catch {
        const mocks: Lesson[] = [
          { id: '1', title: 'Python Lambda', language: 'python', content: "print('Hello Lambda')" },
          { id: '2', title: 'FastAPI', language: 'python', content: 'app = FastAPI()' },
        ];
        const custom: Lesson[] = JSON.parse(localStorage.getItem('custom-lessons') || '[]');
        setLessons([...mocks, ...custom]);
      } finally {
        setLoading(false);
      }
    }
          ...
          <div className="grid">
            {loading
              ? Array(3).fill(0).map((_, i) => (
                  <div key={i} className="glass-panel" style={{ height: '280px', opacity: 0.25, animation: 'fadeIn 1s ease infinite alternate' }} />
                ))
              : lessons.map(lesson => (
